...
class EmbeddingService:
    ...
    async def create_embedding(
        ...
        embedding = await self.repository.create_embedding(
            user_id=user_id,
            name=name,
            files=files,
            status_id=status_id,
        ...
        embedding.vector_db_path = str(base_index_path)
        embedding.index_uid = uid

        logger.info(f"[EmbeddingService] Created embedding_id={embedding.id}, uid={uid}")
        return embedding
    ...
    async def update_embedding_metadata(
        ...
        return await self.repository.update_embedding_metadata(
